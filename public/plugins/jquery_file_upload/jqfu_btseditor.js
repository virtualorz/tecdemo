    ...
    this.init = function (options) {
        ...
        var fileUploader = _this.JqfuBtseditor.find('.jqfubtseditor-btn-input').fileupload({
            ...
            add: function (e, data) {
                ...
                var $item = _this.config.btseditor.bindEvent.jqfuAddPic(_this.config.editor_dest);
                var $context = $item.find('.upload-progress');
                $context.removeClass('progress-bar-danger');
                ...
                if (!valid) {
                    if (_this.config.btseditor.BtsEditor.find('.btseditor-data-item.item-pic.editing').length <= 1) {
                        _this.config.btseditor.BtsEditor.find('.uploading').removeClass('uploading');
                        _this.config.btseditor.BtsEditor.find('.btseditor-data-item.item-pic.editing .btseditor-item-url input').focus();
                    }
                    return false;
                } else {
            ...
            sent: function (e, data) {
                ...
                var $item = data.context.parents('.btseditor-data-item.item-pic:first');
            ...
            stop: function (e) {
                _this.config.btseditor.BtsEditor.find('.uploading').removeClass('uploading');
                _this.config.btseditor.BtsEditor.find('.btseditor-data-item.item-pic.editing').not(':last').each(function () {
                    $(this).data('val_file', $(this).find('.btseditor-item-pic').data('file'));
                    $(this).data('val_url', $(this).find('.btseditor-item-url input').val());
